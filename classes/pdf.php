...
//PRIMERO COMPROBAMOS SI HAY ALGUN MENU O OFERTA EN LA LISTA
        $consulta_ofertas = "SELECT * FROM comandatemp_" . $_SESSION['id_usuario'] . " WHERE idGrupoMenuComandaTemp IS NOT NULL  GROUP BY idGrupoMenuComandaTemp ORDER BY id DESC";
...
//HACEMOS CONSULTA DE LA TABLA TEMPORAL DEL USUARIO
        $comandaTemporal_Query = "SELECT * FROM comandatemp_" . $_SESSION['id_usuario'] . " WHERE idMenuComandaTemp IS NULL";
        ...
        $total_rows_comandaTemp = mysql_num_rows($RegComandasTemp);
...
//COMPROVAMOS SI HAY MENUS Y SI ES ASI SE COLOCAN AL FINAL
        if ( count($registros > 0)){
            ...
            for ($w = 0; $w < count($registros); $w++){                
                $pdf->Cell(125, 15, utf8_decode($registros[$w][0]), 1, 0, 'L', 0);
                $pdf->Cell(30, 15, $registros[$w][1], 1,  0, 'C', 1);
                $pdf->Cell(30, 15, $registros[$w][2].EURO, 1, 0, 'C', 1);
                $pdf->Ln(15);
            }
        }
        ...
        $pdf->Cell(30, 15, $pedidoTotal.EURO, 1, 0, 'C', 1);
        $pdf->Ln(5);
